...
/**
 * UserAnswer 결과를 UserResult 테이블로 옮겨준다.
 * input  : chatId, starId, chatId, starId
 * output : none
 */
const moveResultQuery = ("INSERT INTO UserResult \
						(SELECT chatId, starId, qId, ansId, (SELECT IFNULL(max(try), 0)+1 FROM UserResult WHERE chatId = ? AND starId = ?) as try, NOW() as date FROM UserAnswer WHERE chatId = ? AND starId = ?)");
